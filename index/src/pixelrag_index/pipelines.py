...
def build(config: dict, limit: int | None = None, force: bool = False) -> Path:
    ...
    # Render PDFs. render_pdf names the tile dir after the PDF filename stem,
    # but embed/serve map vectors to articles.json by the sequential position
    # index — non-numeric stems would fall back to hash() article ids and break
    # that mapping. Rename to the position index, like the URL/text branches.
    for idx, doc in pdf_docs:
        ...
        try:
            ...
            src_dir = tiles_dir / f"{Path(doc.path).stem}.png.tiles"
            if src_dir != out_dir and src_dir.exists():
                src_dir.rename(out_dir)
